 ...
 * @author Enaium
 */
class KspArgumentTest {
    @Test
    fun test() {
        val argument = mutableMapOf<String, Any>()
        var content = ""
        argument[DTO_DIRS] = listOf("src/main/kotlin")
        argument[DTO_TEST_DIRS] = listOf("src/test/kotlin")
        ...
        for ((k, v) in argument) {
            ...
            content += """if (ksp.arguments["$k"] != "$value") {"""
            content += "\n"
            content += """error("$k")"""
            content += "\n"
            content += "}"
        ...
        val projectTest = ProjectTest("kspArgument")
